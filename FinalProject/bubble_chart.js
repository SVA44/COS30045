    ...
    var myNodes = rawData.map(function (d) {
      let healthExpenditure = [];
        for (let year = 2015; year <= 2022; year++) {
            healthExpenditure.push(+d["year_" + year]); // Convert to number and push to array
        }
  ...
  var chart = function chart(selector, rawData) {
    ...
    bubbles.enter().append('circle')
      ...
      .on('click', function(event, d) {
        // Use the data parameter 'd' to access the data associated with the clicked bubble
        console.log('Clicked data:', d.id);
        clickbubble(d)
      }); 
